<%@ Page Language="C#"  Inherits="SiteServer.BBS.Pages.Dialog.DisabledUsers"  Trace="False"%>
    ...
    function onSubmit(form) {
        $(form).ajaxSubmit({
            ...
            success: function (data) {
                if (data.isForbid == 'true') {
                    failureMessage('不能对系统用户进行操作');
                }
...
<body>
<form id="form1"  onsubmit="onSubmit(this);return false;" action="?isPostBack=True&publishmentSystemID=<%=Request.QueryString["publishmentSystemID"]%>&forumID=<%=Request.QueryString["forumID"]%>&threadID=<%=Request.QueryString["threadID"]%>&postIDArray=<%=Request.QueryString["postIDArray"]%>&action=<%=Request.QueryString["action"] %>" method="post">
  <div class="container">
    <table width="100%" class="dtb">
      <tbody>
        <tr class="tr3 line">
          <td><p>您确认要 禁止/解除 选中用户么 <strong></strong></p></td>
        </tr>
      </tbody>
    </table>
    <table width="100%" class="dtb">
      <tbody>
        <tr class="tr3 line">
          ...
          <td><label class="mr20">
              <input type="radio" name="userGroupType" value="" />
              解除禁止 </label>
            <label class="mr20">
              <input type="radio" name="userGroupType" value="WriteForbidden" checked="checked"  />
              禁止发言</label>
            <label class="mr20">
              <input type="radio" name="userGroupType" value="ReadForbidden" />
              禁止访问</label></td>
        </tr>
      </tbody>
    </table>
