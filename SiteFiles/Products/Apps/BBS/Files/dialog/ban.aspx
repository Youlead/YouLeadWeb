<%@ Page Language="c#" Inherits="SiteServer.BBS.Pages.Dialog.Ban" Trace="False" %>
...
<body>
<form onsubmit="onSubmit(this);return false;" action="?isPostBack=True&publishmentSystemID=<%=Request.QueryString["publishmentSystemID"]%>&forumID=<%=Request.QueryString["forumID"]%>&threadID=<%=Request.QueryString["threadID"]%>&threadIDArray=<%=Request.QueryString["threadIDArray"]%>&postIDArray=<%=Request.QueryString["postIDArray"]%>&action=<%=Request.QueryString["action"]%>" method="post">
  <div class="container">
    <table width="100%" class="dtb">
      <tbody>
        <tr class="tr3 line">
          <td><p>您确认要 <strong>屏蔽/解除 </strong>选中项么?</p></td>
        </tr>
      </tbody>
    </table>
    <table width="100%" class="dtb">
      <tbody>
        <tr class="tr3 line">
          ...
          <td><label class="mr20">
              <input checked="checked" type="radio" name="isBanned" value="True" />
              屏蔽 </label>
            <label class="mr20">
              <input type="radio" name="isBanned" value="False" />
              解除</label></td>
        </tr>
      </tbody>
    </table>
